...
def eval_dicts_bin(input_file,output_name,goldstandard):
    ...
    sentiws = classification_report(gs[goldstandard], gs['sentiws'],labels=['negative','positive'],output_dict=True)
    sentiws =pd.DataFrame(sentiws)
    sentiws['method'] = 'sentiws_dict'
    
    
    slk = classification_report(gs[goldstandard], gs['slk'],labels=['negative','positive'],output_dict=True)
    slk= pd.DataFrame(slk)
    slk['method'] = 'slk'
    
    bpw = classification_report(gs[goldstandard], gs['bpw'],labels=['negative','positive'],output_dict=True)
    bpw =pd.DataFrame(bpw)
    bpw['method'] = 'bpw_dict'
    
    bawl_r = classification_report(gs[goldstandard], gs['bawl_r'],labels=['negative','positive'],output_dict=True)
    bawl_r =pd.DataFrame(bawl_r)
    bawl_r['method'] = 'bawl_r'
    
    gpc = classification_report(gs[goldstandard], gs['gpc'],labels=['negative','positive'],output_dict=True)
    gpc =pd.DataFrame(gpc)
    gpc['method'] = 'german_polarity_clues'
   
    zs = classification_report(gs[goldstandard], gs['zeroshot'],labels=['negative','positive'],output_dict=True)
    zs =pd.DataFrame(zs)
    zs['method'] = 'zeroshot'
    

    
    guhr_pipe = classification_report(gs[goldstandard], gs['guhr'],labels=['negative','positive'],output_dict=True)
    guhr_pipe =pd.DataFrame(guhr_pipe)
    guhr_pipe['method'] = 'guhr_pipe'
