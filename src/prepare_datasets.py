...
def make_scare():
    ...
    df = df[df['stars']!= 3]
    df = df.sample(100000,replace=False,random_state=2)
    df= df.reset_index(drop=True)
    df['label'] = df['stars'].apply(lambda x: 'positive' if x > 3 else 'negative')
    df.to_feather('scare_reviews_100k.feather')      
...
def make_holidaycheck():
    holiday_check = pd.read_csv('datasets/holidaycheck.clean.filtered.tsv',sep = '\t',usecols=[0,1],names = ['stars','text'],header=None)
    # exclude "neutral" reviews. due to different star rating 3 and 4 star ratings are excluded
    holiday_check =  holiday_check[(holiday_check['stars'] < 3) | (holiday_check['stars'] > 4)]
    holiday_check = holiday_check.sample(100000,replace=False,random_state=2)
    holiday_check = holiday_check.reset_index(drop=True)
    
    holiday_check.to_feather('datasets/holiday_check_100k.feather')
...
def filmstarts():
    ...
    filmstarts['label'] = filmstarts['stars'].apply(lambda x: 'positive' if x > 3 else 'negative')
    filmstarts.to_feather('datasets/filmstarts.feather')
...
def gnd():
    ...
    gnd.to_feather('datasets/gnd.feather')
...
def senti_lit_krit():
    ...
    slk.to_feather('datasets/slk.feather')
